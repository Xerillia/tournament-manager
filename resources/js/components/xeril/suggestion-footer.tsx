import { insertSuggestionToSlot } from '@/routes/slots';
import { listCustomMaps } from '@/routes/tournaments/custom';
...
export default function SuggestionFooter({ tournament, current_id = -1 }: SuggestionFooterProps) {
    return (
        <div className="fixed bottom-0 z-2 flex h-16 w-full items-center justify-center bg-gray-300">
            {current_id === -1 ? (
                <div className="flex h-full items-center bg-gray-500 px-2 text-xl font-semibold text-white">Custom Maps</div>
            ) : (
                <Link
                    href={listCustomMaps(tournament)}
